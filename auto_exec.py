    ...
    try:
        ...
        if not lifecycle.stop_order_id and lifecycle.stop is not None and _should_attempt('stop_last_attempt_at'):
            ...
            try:
                ...
                _append_note(lifecycle, f"STOP_PLACE_ERR {str(e)[:200]}")
                # Re-check broker truth in case the broker accepted despite preview oddities
                open_orders = _extract_open_orders_for_symbol(client, account_id_key, symbol)
                _attach_from_broker()
    ...
    for symbol, lst in list(lifecycles.items()):
        for idx, raw in enumerate(list(lst)):
            ...
            if str(lifecycle.stage or "").upper() == "IN_POSITION":
                try:
                    _ensure_brackets(client, account_id_key, lifecycle.symbol, lifecycle, log_prefix="[GUARD] ")
                except Exception:
                    pass
            ...
            try:
                ...
                if ("1028" in emsg) or ("duplicate order" in emsg.lower()):
                    ...
                    if recovered and recovered.get("orderId"):
                        ...
                        lifecycle.entry_order_id = oid
                        lifecycle.entry_sent_ts = lifecycle.entry_sent_ts or now.isoformat()
                        lifecycle.stage = "ENTRY_SENT"
                        ...
                        lifecycle.last_entry_eval = f"FAILED: duplicate_order_no_recovery ({emsg})"
                        _event_once(cfg, lifecycle, "ENTRY_SEND_FAILED", f"[AUTOEXEC] {symbol} ENTRY SEND FAILED", f"Time (ET): {now.isoformat()}\nSymbol: {symbol}\nEngine: {lifecycle.engine}\n\nEntry placement failed (duplicate w/o recovery): {e}\n")
                else:
                    lifecycle.notes = f"entry_send_failed: {e}"
...
def _reconcile_one(client: ETradeClient, account_id_key: str, state: Dict[str, Any], lifecycle: TradeLifecycle, cfg: AutoExecConfig, fetch_last_price_fn=None) -> None:
    ...
    if lifecycle.stage == "IN_POSITION" and int(lifecycle.filled_qty or 0) > 0:
        try:
            _ensure_brackets(client, account_id_key, lifecycle.symbol, lifecycle, log_prefix="[INVAR] ")
    ...
    if lifecycle.stage == "ENTRY_SENT" and lifecycle.entry_order_id:
        ...
        if int(lifecycle.filled_qty or 0) == 0:
            try:
                _pos = _get_positions_map_cached()
                _q = _pos.get(str(lifecycle.symbol or "").upper(), 0)
                force_filled_qty = int(float(_q)) if _q is not None else 0
            except Exception:
                force_filled_qty = 0
...
""",
            ...
            _ensure_brackets(client, account_id_key, lifecycle.symbol, lifecycle, log_prefix="[AUTOEXEC] ")

            if lifecycle.stop_order_id and lifecycle.tp0_order_id:
                ...
                # Brackets incomplete — keep retrying on future reruns (with backoff inside _ensure_brackets).
                ...
                if not lifecycle.stop_order_id:
                    missing.append("STOP")
                if not lifecycle.tp0_order_id:
                    missing.append("TP0")
                lifecycle.brackets_last_error = ("missing=" + ",".join(missing))[:200]
                _record_activity(state, "BRACKETS_INCOMPLETE", lifecycle, lifecycle.brackets_last_error)
                subj = f"[AUTOEXEC] {lifecycle.symbol} BRACKETS INCOMPLETE ({' & '.join(missing)})"
                _event_once(
                    cfg,
                    lifecycle,
                    f"BRACKETS_INCOMPLETE_{filled_qty}_{lifecycle.brackets_attempts}",
                    subj,
                    f"""Time (ET): {now.isoformat()}\nSymbol: {lifecycle.symbol}\nEngine: {lifecycle.engine}\n\nEntry is filled but bracket set is incomplete.\nOrderId: {lifecycle.entry_order_id}\nFilled qty: {filled_qty}\n\nMissing: {", ".join(missing)}\nKnown STOP oid: {lifecycle.stop_order_id}\nKnown TP0 oid: {lifecycle.tp0_order_id}\n\nLast notes: {lifecycle.notes}\n""",
                )
...
""",
        ...
        pos_qty = None
        try:
            _pm = _get_positions_map_cached()
            pos_qty = int(_pm.get(str(lifecycle.symbol or "").upper(), _pm.get(lifecycle.symbol, 0) or 0) or 0)
        except Exception:
            pos_qty = None
                    ...
                    if (now - entry_dt).total_seconds() >= mins * 60.0:
                        ...
                        if entry_px and entry_px > 0 and last_px is not None:
                            ...
                            if gain_pct >= thresh:
                                ...
                                rem_qty = 0
                                try:
                                    pm = _get_positions_map_cached(state, client, account_id_key) or {}
                                    sym_u = str(lifecycle.symbol or "").upper()
                                    pos = pm.get(sym_u) or pm.get(sym_u.upper()) or {}
                                    rem_qty = int(_coerce_int(pos.get("qty", 0), 0))
                                except Exception:
                                    rem_qty = int(lifecycle.filled_qty or 0)

                                if rem_qty > 0:
                                    oid = _place_sell_close_best_effort(
                                        client,
                                        account_id_key,
                                        lifecycle.symbol,
                                        rem_qty,
                                    ...
                                    if oid:
                                        lifecycle.market_exit_order_id = str(oid)
                                lifecycle.notes = (lifecycle.notes or "") + f" | TIME_PROFIT_CAPTURE gain_pct={gain_pct:.2f}% mins={mins:.1f}"
            ...
            if tp0_px is not None and not lifecycle.tp0_order_id:
                ...
                if ok_attempt and last_px is not None and last_px >= tp0_px:
                    ...
                    try:
                        ...
                        _append_note(lifecycle, f"TP0_PLACE_ERR {str(te)[:220]}")
        except Exception:
            pass
